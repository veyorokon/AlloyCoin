...
#if defined(NDEBUG)
# error "Alloycoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xd1ddcac90c22c6a8916997723c898da175ef8c6cf3cae1900f296f60cc662ac2");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Alloycoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Alloycoin Signed Message:\n";
...
{
    // Alloycoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Alloycoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 60; // Alloycoin: 5 mins
static const int64 nTargetSpacing = 1.0 * 30; // Alloycoin: 30 secs
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Alloycoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Alloycoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xd1ddcac90c22c6a8916997723c898da175ef8c6cf3cae1900f296f60cc662ac2");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "BBC 06/12/2017 Russia protests: Hundreds detained at opposition rallies";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x5e74d85ee53d325db59976c208f42e1a632243c20f99c4f5d18b4c7c35a4ffeb"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Alloycoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AlloycoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AlloycoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AlloycoinMiner(CWallet *pwallet)
{
    printf("AlloycoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("alloycoin-miner");
    ...
    try { loop {
        ...
        printf("Running AlloycoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AlloycoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AlloycoinMiner, pwallet));
}
